...
import { getKudosData, type KudosData, initialKudosData } from '@/lib/utils'

const tabOrder: TabType[] = ['calendar', 'team', 'home', 'tasks', 'kudos']
...
export default function EmployeePage() {
  ...
  const [kudosData, setKudosData] = useState<KudosData>(initialKudosData)
  const [openTaskChatId, setOpenTaskChatId] = useState<string | null>(null)

  const refreshKudosData = useCallback(() => {
    setKudosData(getKudosData())
  }, [])
  ...
  const renderPage = () => {
    switch (activeTab) {
      case 'home':
        return <HomePage isPunchedIn={isPunchedIn} punchInTime={punchInTime} onNavigate={handleTabChange} kudosData={kudosData} />
      case 'tasks':
        return <PingsPage onKudosUpdate={refreshKudosData} onOpenTaskChat={handleOpenTaskChat} />
      case 'kudos':
        return <CoinsPage kudosData={kudosData} onKudosUpdate={refreshKudosData} />
      case 'calendar':
        return <CalendarPage />
      case 'team':
